...
export default async function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-center gap-12 md:gap-20">
            ...
            <Card className="relative aspect-square md:aspect-4/5 border-4 border-stone-900 bg-muted overflow-hidden py-0">
              <Image
                src="/pasta-dish.png"
                alt="Sample plated pasta (swap this image in /public if you want)"
                width={500}
                height={500}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {FEATURES.map((feature, index) => {
              ...
              return (
                <Card
                  key={index}
                  className="border-2 border-border bg-white hover:border-orange-600 hover:shadow-lg transition-all group py-0"
                >
                  <CardContent className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <div className="border-2 border-border bg-orange-50 p-3 group-hover:border-orange-600 group-hover:bg-orange-100 transition-colors">
      ...
      {/* How It Works */}
      <section className="py-24 px-4 border-y-2 border-stone-900 bg-stone-900 text-stone-50">
            ...
            {HOW_IT_WORKS_STEPS.map((item, i) => (
              <div key={i}>
                ...
                {i < HOW_IT_WORKS_STEPS.length - 1 && (
                  <hr className="my-8 border-0 h-px bg-stone-200" />
                )}
              </div>
            ))}
          </div>
